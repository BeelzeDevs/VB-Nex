Attribute VB_Name = "Shipping_Mark_EXCELL_Final"
Sub generador_Shipping_Mark_Excell()
    ...
    Dim matrz(1 To 12, 300) As String
    Dim cantidadEtiquetas As Integer
    Dim largo_palab(0 To 4, 300) As Integer
    ws.Cells.Clear
    Dim po As String
    ...
    Call buscarproduct(producto, po)
    ...
    Call ponerceropalab(largo_palab)
...
'        matrz(12, j) = ActiveCell.Offset(j, -14) ' Model
        largo_palab(0, j) = Len(matrz(1, j))  'CANTIDAD L nro de parte
        largo_palab(1, j) = Len(matrz(4, j))  'CANTIDAD L carton No. CODIGO
        largo_palab(2, j) = Len(matrz(5, j))  'CANTIDAD L Qty Carton
        largo_palab(3, j) = Len(po) 'CANTIDAD L nro de parte


    Next j

    Sheets("Shipping mark").Select
    Call formato(rng, rngoff, matrz, cantidadEtiquetas, largo_palab, po)
...
Sub formato(ByRef celda As Range, ByRef celdaoffset As Range, ByRef m() As String, ByRef cant As Integer, ByRef largo() As Integer, ByRef po As String)
    ...
    For x = 0 To cant - 1
                ...
                celda.Offset(0, 1).Value = m(1, x) & Chr(10) & m(1, x)  'nro de parte CODIGO
                ...
                celda.Offset(2, 3).Value = m(4, x) & Chr(10) & m(4, x)  'carton No. CODIGO

                celda.Offset(3, 3).Value = m(5, x) & Chr(10) & m(5, x)  'Qty Carton
                celda.Offset(3, 1).Value = po & Chr(10) & po      'PO Number
                'celda.Offset(4, 1).Value = m(11, x) ' Brand
                'celda.Offset(5, 1).Value = m(12, x) ' Model
                celda.Offset(7, 1).Value = m(10, x)  ' ORIGEN

                Call formato2(celda, celdaoffset, largo, x)
...
Sub formato2(ByRef celda As Range, ByRef celdaoffset As Range, ByRef largo() As Integer, ByVal nro As Integer)
    Sheets("Shipping mark").Select
...
'    celda.Offset(5, 0).Value = "Model:"
    ...
    'INTENTO DINAMICO 1
    With celda.Offset(0, 1).Characters(Start:=1, Length:=largo(0, nro) + 1).Font 'B1:D1
        .Name = "code 128"
        .FontStyle = "Normal"
        .Size = 16
        .Shadow = False
        ...
        .TintAndShade = 0
    End With
    With celda.Offset(0, 1).Characters(largo(0, nro) + 2, Length:=largo(0, nro)).Font 'B1:D1
        .Name = "Century Gothic"
        .FontStyle = "Normal"
        .Size = 12
        .Strikethrough = False
    ...
    With celda.Offset(3, 1).Characters(Start:=1, Length:=largo(3, nro) + 1).Font   'B4
        .Name = "Code 128"
        .Size = 16
        .Shadow = False
        .Strikethrough = False
    ...
    End With
     With celda.Offset(3, 1).Characters(Start:=largo(3, nro) + 2, Length:=largo(3, nro)).Font    'B4
        .Name = "Century Gothic"
        .Size = 12
        .Shadow = False
        .Strikethrough = False
    ...
    End With
    With celda.Offset(2, 3).Characters(Start:=1, Length:=largo(1, nro) + 1).Font    'D3
        .Name = "Code 128"
        .Size = 16
        .Shadow = False
        .Strikethrough = False
    ...
    End With
     With celda.Offset(2, 3).Characters(Start:=largo(1, nro) + 2, Length:=largo(1, nro)).Font    'D3
        .Name = "Century Gothic"
        .Size = 12
        .Shadow = False
        .Strikethrough = False
    ...
    With celda.Offset(3, 3).Characters(Start:=1, Length:=largo(2, nro) + 1).Font    'D4
        .Name = "Code 128"
        .Size = 16
        .Shadow = False
        .Strikethrough = False
    ...
    End With
     With celda.Offset(3, 3).Characters(Start:=largo(2, nro) + 2, Length:=largo(2, nro)).Font    'D4
        .Name = "Century Gothic"
        .Size = 12
        .Shadow = False
        .Strikethrough = False
...
Sub ponerceropalab(ByRef m() As Integer)
    For i = 0 To 4
...
Sub buscarproduct(ByVal producto As String, ByRef po As String)
producto = UCase(producto)


If (StrComp("BICICLETAS", producto, 0) = 0 Or StrComp("BICICLETA", producto, 0) = 0) Then
     'po = "EL06-01YT"
     'po = "MU01BC-13"

     po = "MU01BC-14" ' BICICLETA JC ORDEN 14
End If
If (StrComp("KETTLE", producto, 0) = 0 Or StrComp("PAVA", producto, 0) = 0 Or StrComp("PAVAS", producto, 0) = 0) Then
    po = "EL05-02JL"
End If
If (StrComp("VACUUM CLEANER ROBOT", producto, 0) = 0 Or StrComp("VACUUM", producto, 0) = 0 Or StrComp("VACUUM CLEANER", producto, 0) = 0 Or StrComp("CLEANER ROBOT", producto, 0) = 0) Then
    po = "nada"
End If
If (StrComp("ELECTRIC OVEN", producto, 0) = 0 Or StrComp("OVEN", producto, 0) = 0) Then
    po = "nada"
End If
